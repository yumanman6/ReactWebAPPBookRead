...
import { Button, ErrorBlock } from "@/base";
...
const ChapterContent: React.FC = () => {
    ...
    const { error, data:chapters } = useRequest<IChapterInfo[]>({ url: api.getChapter(bookId as string, chapterId as string)});
    ...
    return <div 
        ...
        <div className={styles.pagination}>
            <Button onClick={onPrev} disabled={isFirst}>上一章</Button>
            <Button onClick={onNext} disabled={isLast}>下一章</Button>
        </div>
    </div>
}
