...
import { SidebarItemProps } from '@/base/sidebar/sidebar-item';

import { traverseReactNode } from '@/base/utils/traverse-react-node';
...
const Sidebar: React.FC<SidebarProps> = React.memo((props) => {
  ...
  traverseReactNode(props.children, (child) => {
    ...
    if (!child.key) return;
    items.push(child as React.ReactElement<SidebarItemProps>);
  });

  const onSetActive = (e: React.MouseEvent) => {
  ...
  return <div className={classPrefix}>
    <div className={`${classPrefix}-items`} style={props.style}>
      {
        items.map((item) => {
          ...
          return (
            <div className={cx(`${classPrefix}-item`, {
            ...
            onClick={onSetActive}
            >
              <div className={`${classPrefix}-item-title`} data-key={item.key}>{item.props.title}</div>
            </div>
          )
        })
      }
    ...
    <div className={`${classPrefix}-content`}>
        {
          items.map((item) => (
            <div className={`${classPrefix}-content-item`} key={item.key} style={{
